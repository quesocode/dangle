...
@implementation DangleChatViewController
...
- (void) connect:(void(^)( BOOL success , NSError *error ))completion;
{
    ...
    [DangleAPI connect:^(BOOL success, NSError *error) {
        ...
        ALog(@"DangleChatViewController - connect:", @[@"success", MBOOL(success),
                                       @"error", MObj(error),
                                       @"-",
                                       @"conversation", MObj([DangleChatroom activeRoom].conversationID),
...
{
    [super viewDidLoad];
    self.senderDisplayName = [DangleUser user].displayName;
    self.senderId = [DangleUser user].userID;
    self.data = [[DangleMessagesData alloc] init];
...
//
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Details" style:UIBarButtonItemStyleBordered target:self action:@selector(receiveMessagePressed:)];
...
- (void)receiveMessagePressed:(UIBarButtonItem *)sender
{
    ...
    if (!copyMessage) {
        copyMessage = [JSQMessage messageWithSenderId:kDAUserId2
                                          displayName:kDAUserDisplayName2
                                                 text:@"First received!"];
    }
...
- (id<JSQMessageBubbleImageDataSource>)collectionView:(JSQMessagesCollectionView *)collectionView messageBubbleImageDataForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    id bubble = [self.data bubbleForUserID:message.senderId outgoing:[message.senderId isEqualToString:self.senderId]];
...
{
    return 10.0f; // This is the minimum inter item spacing, can be more
}
